...
def convert(source, to, format=None, extra_args=(), encoding='utf-8'):
    ...
    It's possible to pass `extra_args` if needed. 
    In case `format` is not provided, it will try to invert the format 
      based on given `source`.
    Raises OSError if pandoc is not found! 
    Make sure it has been installed and is available at path.
    '''
    return _convert(_read_file, _process_file,
                    source, to, format, extra_args, encoding=encoding)
...
def _convert(reader, processor, source, to, format=None, extra_args=(), encoding=None):
    ...
    (from_list, to_list) = get_pandoc_formats()
    (from_formats, to_formats) = (', '.join(from_list), ', '.join(to_list))

    if format not in from_formats:
        raise RuntimeError('Invalid input format! Expected one of: ' + from_formats)

    if to not in to_formats:
        raise RuntimeError('Invalid to format! Expected one of: ' + to_formats)
...
def get_pandoc_formats():
    ...
    help_text = proc.communicate()[0].decode().splitlines(False)
